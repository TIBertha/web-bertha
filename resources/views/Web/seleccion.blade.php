...
@section('seo')
    ...
    <meta property="og:url" content="{{ route('pedidos-country', ['lang' => $lang, 'country' => $country]) }}" />
    <meta property="og:image" content="{{ asset('img/og.png') }}" />
@endsection

@section('content')
    <div id="seleccion" data-url="{{ url('/') }}" data-session="{{ $checksession }}" data-country="{{$country}}" data-lang="{{$lang}}"></div>
@endsection
