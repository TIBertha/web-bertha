...
export default function ModalComoFunciona({url, showModal, setShowModal, closeModal, country, lang}) {
    ...
    return (
        <Modal className={'bertha-modal-aviso'} size="xl" scrollable={true} show={showModal} onHide={() => setShowModal(false)} centered={true} backdrop="static" keyboard={false}>
            ...
            <ModalBody className={'no-select-text'}>
                <FrameComoFunciona url={url} showButton={false} country={country} lang={lang}/>
            </ModalBody>

            <ModalFooter className="center-button-modal">
                <a className="btn btn-lg bertha-pink-button w-100 font-weight-bold text-white" onClick={()=>closeModal()} >{textViewWorkers[lang]}</a>
            </ModalFooter>
